...
function gpsWorker(socket, position) {
  ...
  // let lat = Number((position.coords.latitude - 0.412375).toFixed(6));
  // let lon = Number((position.coords.longitude + 0.502332).toFixed(6));
...
function main() {
  document.getElementById('eqpls-username').innerText =
    window.common.auth.username;
  document.getElementById('eqpls-access-token').innerText =
    window.common.auth.accessToken;

  window.common.rest.get(
    '/uerp/v1/demo/device/cart?$archive&$size=100&$orderby=name&$order=asc',
    (carts) => {
      console.log(carts);
