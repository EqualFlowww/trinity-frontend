...
window.common.auth.checkUserInfo = (resultHandler, errorHandler) => {
  ...
  if (window.common.auth.username) {
    window.common.auth.userInfo = JSON.parse(
    ...
    if (window.common.auth.accessToken) {
      fetch(
        `/auth/realms/${window.common.auth.getRealm()}/protocol/openid-connect/userinfo`,
        {
        ...
        .then((userInfo) => {
          ...
          localStorage.setItem(
            'authUserInfo',
            JSON.stringify(window.common.auth.userInfo)
          );
          if (resultHandler) {
            resultHandler();
          }
  ...
  if (DEFAULT_DATA_SERVICE) {
    ...
    if (window.common.auth.dataToken) {
      ...
      fetch('/minio/ui/api/v1/login')
        ...
        .then((data) => {
          fetch(data.redirectRules[0].redirect)
            .then((res) => {
              if (res.ok) {
                return res.json();
              }
...
window.common.auth.loginTermService = (resultHandler, errorHandler) => {
  if (DEFAULT_TERM_SERVICE) {
    ...
    if (window.common.auth.termToken) {
      fetch(
        `/guacamole/api/session/data/postgresql/users/${window.common.auth.username}`,
        {
      ...
      window.location.hash.indexOf('id_token=')
    ) {
...
window.common.auth.login = (resultHandler, errorHandler) => {
  if (resultHandler) {
    window.common.auth.loginResultHandler = resultHandler;
  } else {
    resultHandler = window.common.auth.loginResultHandler;
  }
  if (errorHandler) {
    window.common.auth.loginErrorHandler = errorHandler;
  } else {
    errorHandler = window.common.auth.loginErrorHandler;
  }
    ...
    window.common.auth.checkUserInfo(
      ...
      () => {
        ...
        window.common.auth.keycloak.onAuthSuccess = () => {
          localStorage.setItem(
            'authAccessToken',
            window.common.auth.keycloak.token
          );
          localStorage.setItem(
            'authRefreshToken',
            window.common.auth.keycloak.refreshToken
          );
          localStorage.setItem(
            'authIdToken',
            window.common.auth.keycloak.idToken
          );
          window.common.auth.checkUserInfo(() => {
            window.common.auth.loginDataService(() => {
              window.common.auth.loginTermService(() => {
...
window.common.auth.logout = () => {
  ...
  if (idToken) {
    window.location.replace(
      `/auth/realms/${window.common.auth.getRealm()}/protocol/openid-connect/logout?id_token_hint=${idToken}&post_logout_redirect_uri=/`
    );
  } else {
    window.location.replace('/');
  }
...
window.common.wsock.connect = (
  ...
  try {
    socket = new WebSocket(
      `wss://${DEFAULT_ENDPOINT}${url}?org=${window.common.auth.realm}&token=${window.common.auth.accessToken}`
    );
  } catch (e) {
    if (errorHandler) {
      errorHandler(e);
  ...
  socket.onmessage = (event) => {
    console.log('ws:recv');
    if (recvHandler) {
      recvHandler(JSON.parse(event.data));
    }
  };
  socket.onopen = (event) => {
    console.log('ws:open');
...
window.common.data._set_object_functions_ = (bucket, object) => {
  if (object.etag !== undefined) {
    ...
    object.getObjectList = (resultHandler, errorHandler) => {
      fetch(
        `/minio/ui/api/v1/buckets/${bucket}/objects?prefix=${btoa(object.name)}`
      )
        .then((res) => {
          if (res.ok) {
            return res.json();
  ...
  fetch('/minio/ui/api/v1/buckets')
    ...
    .then((data) => {
      ...
      buckets.forEach((object) => {
        ...
        object.getObjectList = (resultHandler, errorHandler) => {
          fetch(`/minio/ui/api/v1/buckets/${object.name}/objects`)
            .then((res) => {
              if (res.ok) {
                return res.json();
              }
            ...
            .then((data) => {
              let objects = data.objects ? data.objects : [];
              objects.forEach((object) => {
                window.common.data._set_object_functions_(object.name, object);
              });
              if (resultHandler) {
                resultHandler(objects);
              }
      ...
      if (resultHandler) {
        resultHandler(buckets);
      }
    });
};

window.common.data.getObjectList = (
  bucket,
  path,
  resultHandler,
  errorHandler
  ...
  fetch(`/minio/ui/api/v1/buckets/${bucket}/objects?${prefix}`)
    .then((res) => {
      if (res.ok) {
        return res.json();
      }
...
window.common.data.upload = (
  ...
  if (files.length > 0) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      coros.push(
        fetch(
          `/minio/ui/api/v1/buckets/${bucket}/objects/upload?prefix=${btoa(prefix)}`,
          {
            method: 'POST',
            body: form,
          }
...
window.common.data.delete = (bucket, path, resultHandler, errorHandler) => {
  fetch(`/minio/ui/api/v1/buckets/${bucket}/objects?prefix=${btoa(path)}`, {
    ...
    if (res.ok) {
      if (resultHandler) {
        resultHandler(bucket, path, res);
      }
    } else if (errorHandler) {
      errorHandler(bucket, path, res);
    }
  });
};
...
window.common.term.openNewWindow = (username, hostname, hostport) => {
  ...
  window.open(
    `/static/html/console.html?username=${username}&hostname=${hostname}&hostport=${hostport}`,
    '_blank',
...
window.common.term.openOnDom = (dom, username, hostname, hostport) => {
  hostport = hostport ? hostport : '22';
  fetch(
    `/guacamole/api/session/ext/quickconnect/create?token=${window.common.auth.termToken}`,
    {
      method: 'POST',
...
window.common.term.connectToSSH = (dom, id) => {
  ...
  options +=
    '&GUAC_AUDIO=audio/L8&GUAC_AUDIO=audio/L16&GUAC_IMAGE=image/jpeg&GUAC_IMAGE=image/png&GUAC_IMAGE=image/webp&GUAC_TIMEZONE=Asia/Seoul';
